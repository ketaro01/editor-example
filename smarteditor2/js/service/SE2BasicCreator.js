...
function createSEditor2(elIRField, htParams, elSeAppContainer) {
  ...
  var oWYSIWYGIFrame = jindo.$$.getSingle(
    "IFRAME.se2_input_wysiwyg",
    elEditingArea
  );
  var oIRTextarea = elIRField
    ? elIRField
    : jindo.$$.getSingle("TEXTAREA.blind", elEditingArea);
  var oHTMLSrc = jindo.$$.getSingle(
    "TEXTAREA.se2_input_htmlsrc",
    elEditingArea
  );
  ...
  oEditor.registerPlugin(
    new nhn.husky.CorePlugin(htParams ? htParams.fOnAppLoad : null)
  );
  ...
  oEditor.registerPlugin(
    new nhn.husky.SE2M_EditingModeChanger(elAppContainer, htConversionMode)
  ); // 모드간 변경(Editor, HTML, Text)
  ...
  oEditor.registerPlugin(
    new nhn.husky.SE_EditingAreaVerticalResizer(
      elAppContainer,
      htConversionMode
    )
  ); // 편집영역 리사이즈
  ...
  oEditor.registerPlugin(
    new nhn.husky.SE2M_FontSizeWithLayerUI(elAppContainer)
  ); // 글꼴크기

  oEditor.registerPlugin(new nhn.husky.SE2M_LineStyler());
  oEditor.registerPlugin(new nhn.husky.SE2M_ExecCommand(oWYSIWYGIFrame));
  oEditor.registerPlugin(
    new nhn.husky.SE2M_LineHeightWithLayerUI(elAppContainer)
  ); // 줄간격
  ...
  oEditor.registerPlugin(
    new nhn.husky.MessageManager(oMessageMap, htParams.I18N_LOCALE)
  );
  ...
  if (window.frameElement) {
    oEditor.registerPlugin(
      new nhn.husky.SE_OuterIFrameControl(elAppContainer, 100)
    );
  }

  oEditor.registerPlugin(
    new nhn.husky.SE_ToolbarToggler(elAppContainer, htParams.bUseToolbar)
  );
  oEditor.registerPlugin(
    new nhn.husky.SE2M_Accessibility(elAppContainer, htParams.I18N_LOCALE)
